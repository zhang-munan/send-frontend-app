...
export function getProductList() {
  return request<ProductInfo[]>({ url: `${PRODUCT_PREFIX}/list`, method: 'GET' })
}

/** 获取商品详情 */
export function getProductDetail(id: number) {
  return request<ProductInfo>({ url: `${PRODUCT_PREFIX}/detail`, method: 'GET', data: { id } })
}
...
export function getBalance() {
  return request<UserBalance>({ url: `${BALANCE_PREFIX}/info`, method: 'GET' })
}
...
export function createOrder(data: CreateOrderByProduct | CreateOrderBySend) {
  return request<OrderInfo>({ url: `${ORDER_PREFIX}/create`, method: 'POST', data })
}
...
export function payOrder(orderId: number, payMethod: number) {
  return request<WxPayParams | BalancePayResult>({
...
export function queryOrderStatus(orderId: number) {
  return request<Pick<OrderInfo, 'id' | 'orderNo' | 'status' | 'payAmount' | 'payTime'>>({
...
export function getOrderDetail(id: number) {
  return request<OrderInfo>({ url: `${ORDER_PREFIX}/detail`, method: 'GET', data: { id } })
}

/** 订单列表 */
export function getOrderList(page = 1, size = 10) {
  return request<{ list: OrderInfo[], total: number, page: number, size: number }>({
